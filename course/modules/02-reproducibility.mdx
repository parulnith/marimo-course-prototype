...
## Install and Start marimo

From this point in the course, you will use marimo directly. You need Python and a terminal. The [official installation guide](https://docs.marimo.io/getting_started/installation/) recommends installing marimo inside a virtual environment so its packages do not conflict with other projects.
...
Install marimo in the active environment:

```bash
pip install marimo
```

If you use `uv`, you can add marimo to a project instead:
...
Run the introductory tutorial:

```bash
marimo tutorial intro
```

If you installed marimo with `uv`, run:
...
Use `edit` when you want to change a notebook. The command creates the file if it does not exist:

```bash
marimo edit my_notebook.py
```

Use `run` when you want to open a notebook without the editing tools:

```bash
marimo run my_notebook.py
```

Later in this module, you will also use `--sandbox`. This option opens the notebook in an isolated environment and installs the dependencies recorded inside the notebook file.

<TryIt>
Install marimo, then run `marimo tutorial intro`. Once the tutorial opens, edit one cell and run it with the play button. Keep the terminal open while you work. Return to the terminal and press `Ctrl + C` when you want to stop marimo.
</TryIt>
...
Reproducibility requires more than rerunning code. You must also control the Python environment and record the dependencies that produced the result. marimo can store notebook dependencies for an isolated sandbox, while its Python file format keeps Git changes focused on the code. These features make experiments easier to reproduce, review, and prepare for production.
