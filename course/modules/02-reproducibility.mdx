...
marimo provides several ways to manage the environment and dependencies used by a notebook. You can use an existing Python environment, give one notebook its own isolated environment, or use a shared environment across a project.

In this section, we will try each approach and see when it is useful.

### 1. Using your current Python environment

When you start marimo normally, the notebook uses the Python environment from which marimo was launched. If you import a package that is missing, marimo can prompt you to install it with your selected package manager. The package is then installed in that active environment.
...
The Packages panel shows which package versions are installed in the current sandbox environment.
</p>
...
1. Replace the starter cell's contents with the following code:
...
<MarimoEmbed
  notebook="/notebooks/2_2_active_environment_starter.html"
...
Sandbox mode gives a notebook its own isolated Python environment. marimo uses `uv` to create the environment and install the dependencies recorded in the notebook. 
...
You can enable sandbox mode when you open the notebook from a terminal.
...
<Callout title="Installing an exact package version">
In the Packages panel, enter the package name followed by `==` and the required version. For example, `pandas==2.3.3` installs pandas 2.3.3. In a local notebook opened with `--sandbox`, installing or removing a package from this panel also updates the dependencies recorded in the notebook file.
</Callout>

<TryIt>
Use the notebook below to inspect its sandbox environment.

1. Run the notebook and note the Python, scikit-learn, and pandas versions printed in the outputs.
2. Open the **Packages** panel in the left sidebar.
3. Find pandas and scikit-learn in the package list. Confirm that their installed versions match the versions printed by the notebook.
4. Switch to the **Tree** view and expand a package to see the packages it depends on.

The direct packages and their exact versions come from the dependency block stored in the notebook file. The Tree view also reveals transitive dependencies that those packages installed.
</TryIt>
