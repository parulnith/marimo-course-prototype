...
The image below will compare the actual Git diff produced by the same code change in a Jupyter notebook and a marimo notebook.

<ImagePlaceholder
  title="Jupyter and marimo Git diff comparison"
  description="Replace this placeholder with an image of the actual Git diffs for the same one-line change."
/>

When reviewing a marimo diff, also check changes to a cell's function parameters and return statement. These show which values the cell receives and which values it provides to other cells.
