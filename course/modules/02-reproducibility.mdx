...
We change only `power = 3` to `power = 4`in both notebooks, rerun the code and save each notebook. While the logic change is the same, the Git diffs look very different.
...
The reason is that a Jupyter notebook is stored as a JSON file. It can contain code, execution counts, metadata, notebook structure, and outputs. Rerunning the notebook changes the parameter, the execution count, and the saved plot data. The plot is stored as a long encoded string, which is why the raw diff is much larger than the visible code change.

A marimo notebook, on the other hand, is stored as a Python file. Its rendered plot is not saved in the notebook file, so the diff stays focused on the change from power = 3 to power = 4.
...
Since a marimo notebook is a Python file, you can open it in VS Code and review it with standard Python and Git tools. Additionally, you can also install the [marimo extension for VS Code](https://marketplace.visualstudio.com/items?itemName=marimo-team.vscode-marimo) to open and edit the file as a notebook.
...
molab notebooks run in [CoreWeave sandboxes](https://www.coreweave.com/products/coreweave-sandboxes). By default, each notebook has 4 CPUs and 32 GB of RAM. This gives you more compute than a typical browser notebook without requiring you to configure a remote machine. For work that needs a GPU, you can attach an NVIDIA RTX Pro 6000 Blackwell GPU with 96 GB of VRAM and 125 TFLOPS to a notebook.
